...
function help() {
  ...
  console.log(
    "  echo 'create a javascript code fragment that prints first 10 fibonacci numbers' | gpt --temperature 0.5 --thread-count 24 --model alphamonarch-7b.Q4_0.gguf | codedown javascript | node"
  );
  console.log(
...
function main() {
  ...
  if (userPrompt.length > 0) {
    ...
    console.error("Direct user input", augmentedPrompt.join("\n"));
    ...
    if (args["--help"]) {
      ...
      readlineInterface.on("line", (input) => {
        console.error(`Piped user prompt: ${input}`);
        source.push(input);
      });
